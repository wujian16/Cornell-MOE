...
function [results] = KISSGP_CFKG(hypers)
sd = 3; rand('seed',sd), randn('seed',sd)       % set a seed for reproducability

a = 0.3; b = 1.2; f = @(x) a*x + b + sin(x);               % underlying function
n = int32(1e5*hypers(4)); sn = 0.5;          % number of training points, noise standard deviation
...
ng = int32(1000*hypers(5)); xg = linspace(-6,8,ng)'; covg = {@apxGrid,{cov},{xg}};% grid prediction

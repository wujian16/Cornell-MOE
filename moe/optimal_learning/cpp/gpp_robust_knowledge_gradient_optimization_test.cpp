...
\endrst*/
class PingRobustKnowledgeGradient final : public PingableMatrixInputVectorOutputInterface {
 public:
  constexpr static char const * const kName = "Robust KG with MC integration";

  PingRobustKnowledgeGradient(TensorProductDomain domain, GradientDescentParameters& optimizer_parameters,
  ...
  virtual void EvaluateAndStoreAnalyticGradient(double const * restrict points_to_sample, double * restrict gradients) noexcept override OL_NONNULL_POINTERS_LIST(2) {
    ...
    RobustKnowledgeGradientEvaluator<TensorProductDomain>::StateType twoei_state(twoei_evaluator_, points_to_sample, points_being_sampled_.data(),
 ...
 private:
  ...
  RobustKnowledgeGradientEvaluator<TensorProductDomain> twoei_evaluator_;

  OL_DISALLOW_DEFAULT_AND_COPY_AND_ASSIGN(PingRobustKnowledgeGradient);
};
...
/*!\rst
  ...
  The output of KG is a scalar.
\endrst*/
class PingRobustPosteriorMean final : public PingableMatrixInputVectorOutputInterface {
 public:
  constexpr static char const * const kName = "Robust Posterior Mean";

  PingRobustPosteriorMean(double const * restrict lengths, double const * restrict points_sampled, double const * restrict points_sampled_value,
  ...
  virtual void EvaluateAndStoreAnalyticGradient(double const * restrict points_to_sample, double * restrict gradients) noexcept override OL_NONNULL_POINTERS_LIST(2) {
    ...
    PosteriorCVAREvaluator::StateType ps_state(ps_evaluator_, 0, points_to_sample, configure_for_gradients);

    ps_evaluator_.ComputeGradPosteriorCVAR(&ps_state, grad_PS_.data());
  ...
  virtual void EvaluateFunction(double const * restrict points_to_sample, double * restrict function_values) const noexcept override OL_NONNULL_POINTERS {
    ...
    PosteriorCVAREvaluator::StateType ps_state(ps_evaluator_, 0, points_to_sample, configure_for_gradients);
    *function_values = ps_evaluator_.ComputePosteriorCVAR(&ps_state);
  }
 ...
 private:
  ...
  GaussianProcess gaussian_process_;
  //! expected improvement evaluator object that specifies the parameters & GP for KG evaluation
  PosteriorCVAREvaluator ps_evaluator_;

  OL_DISALLOW_DEFAULT_AND_COPY_AND_ASSIGN(PingRobustPosteriorMean);
};
...
\endrst*/
template <typename RKGEvaluator>
OL_WARN_UNUSED_RESULT int PingRKGTest(int num_to_sample, int num_being_sampled, double epsilon[2],
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    RKGEvaluator twoei_evaluator(domain, gd_params, lengths.data(), KG_environment.points_being_sampled(), KG_environment.points_sampled(),
  ...
  if (total_errors != 0) {
    OL_PARTIAL_FAILURE_PRINTF("%s (%d,%d-rKG) gradient pings failed with %d errors\n", RKGEvaluator::kName, num_to_sample, num_being_sampled, total_errors);
  } else {
    OL_PARTIAL_SUCCESS_PRINTF("%s (%d,%d-rKG) gradient pings passed\n", RKGEvaluator::kName, num_to_sample, num_being_sampled);
  }
...
/*!\rst
  ...
  \return
    number of ping/test failures
\endrst*/
template <typename RPSEvaluator>
OL_WARN_UNUSED_RESULT int PingRPSTest(int num_to_sample, double epsilon[2], double tolerance_fine, double tolerance_coarse, double input_output_ratio) {
  int total_errors = 0;
  int errors_this_iteration;
  const int dim = 3;
  ...
  for (int i = 0; i < 50; ++i) {
    ...
    RPSEvaluator PS_evaluator(lengths.data(), KG_environment.points_sampled(),

...
namespace {
double ComputeRobustKnowledgeGradientMCMCWrapper(GaussianProcessMCMC& gaussian_process_mcmc,
  ...
  std::vector<typename RobustKnowledgeGradientState<TensorProductDomain>::EvaluatorType> evaluator_vector;
  RobustKnowledgeGradientMCMCEvaluator<TensorProductDomain> vf_evaluator(gaussian_process_mcmc, 0, input_container_discrete.points_to_sample.data(),
                                                                   num_pts, max_int_steps, domain, gradient_descent_parameters,
                                                                   best_so_far_list.data(), factor, &evaluator_vector);

  std::vector<typename RobustKnowledgeGradientEvaluator<TensorProductDomain>::StateType> state_vector;
  RobustKnowledgeGradientMCMCEvaluator<TensorProductDomain>::StateType vf_state(vf_evaluator, input_container.points_to_sample.data(),
  ...
  return vf_evaluator.ComputeValueFunction(&vf_state);
}

boost::python::list ComputeGradRobustKnowledgeGradientMCMCWrapper(GaussianProcessMCMC& gaussian_process_mcmc,
                                                            const boost::python::object& optimizer_parameters,
                                                            const boost::python::list& domain_bounds,
                                                            const boost::python::list& discrete_pts,
                                                            const boost::python::list& points_to_sample,
  ...
  std::vector<typename RobustKnowledgeGradientState<TensorProductDomain>::EvaluatorType> evaluator_vector;
  RobustKnowledgeGradientMCMCEvaluator<TensorProductDomain> vf_evaluator(gaussian_process_mcmc, num_fidelity, input_container_discrete.points_to_sample.data(),
                                                                   num_pts, max_int_steps, domain, gradient_descent_parameters,
                                                                   best_so_far_list.data(), factor, &evaluator_vector);
  ...
  RobustKnowledgeGradientMCMCEvaluator<TensorProductDomain>::StateType vf_state(vf_evaluator, input_container.points_to_sample.data(),
                                                                          ...
                                                                          gaussian_process_mcmc.num_derivatives(), configure_for_gradients,
                                                                          randomness_source.normal_rng_vec.data(), &state_vector);
  vf_evaluator.ComputeGradValueFunction(&vf_state, grad_KG.data());
...
boost::python::list MultistartRobustKnowledgeGradientMCMCOptimizationWrapper(const boost::python::object& optimizer_parameters,
                                                                       const boost::python::object& optimizer_parameters_inner,
                                                                       GaussianProcessMCMC& gaussian_process_mcmc,
                                                                       const boost::python::list& domain_bounds,
                                                                       const boost::python::list& discrete_pts,

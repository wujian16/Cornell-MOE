...
*/
void SquareExponential::Covariance(double const * restrict point_one, int const * restrict derivatives_one, int length_one,
    ...
    for (int i = 0; i < length_two; ++i) {
        ...
        cov[(i+1)*(1+length_one)] = (point_one[index] - point_two[index])/lengths_sq_[index]*cov[0];
    }
    for (int i = 0; i < length_one; ++i) {
        for (int j = 0; j < length_two; ++j) {
            index1 = derivatives_one[i];
            index2 = derivatives_two[j];
            cov[(i+1)+(j+1)*(1+length_one)] = (point_two[index1] - point_one[index1])/lengths_sq_[index1] *
                                              (point_one[index2] - point_two[index2])/lengths_sq_[index2] * cov[0];
            if(index1 == index2){
                   cov[(i+1)+(j+1)*(1+length_one)] += cov[0]/lengths_sq_[index2];
            }
        }
    }
}
...
*/
void SquareExponential::GradCovariance(double const * restrict point_one, int const * restrict derivatives_one, int length_one,
                                       double const * restrict point_two, int const * restrict derivatives_two, int length_two,
  ...
  for (int i = 0; i < dim_; ++i) {
      ...
      for (int n =0; n < length_two; ++n){
          index2 = derivatives_two[n];
          grad_cov[i + (n+1)*dim_*(length_one+1)] = (point_two[i] - point_one[i])/lengths_sq_[i] *
                                                    (point_one[index2] - point_two[index2])/lengths_sq_[index2]*cov;
          if (i == index2){
              grad_cov[i + (n+1)*dim_*(length_one+1)] += cov/lengths_sq_[index2];
          }
      }
      for (int m = 0; m < length_one; ++m){
          index1 = derivatives_one[m];
          for (int n = 0; n < length_two; ++n){
              index2 = derivatives_two[n];
              grad_cov[i+ (m+1)*dim_ + (n+1)*dim_*(length_one+1)] = (point_two[index1] - point_one[index1])/lengths_sq_[index1] *
                                                                    (point_one[index2] - point_two[index2])/lengths_sq_[index2];
              if (index1 == index2){
                  grad_cov[i+ (m+1)*dim_ + (n+1)*dim_*(length_one+1)] += 1/lengths_sq_[index1];
              }
              grad_cov[i+ (m+1)*dim_ + (n+1)*dim_*(length_one+1)] *= (point_two[i] - point_one[i])/lengths_sq_[i];
              if (index1 == i){
                  grad_cov[i+ (m+1)*dim_ + (n+1)*dim_*(length_one+1)] -= (point_one[index2] - point_two[index2])/
                                                                         (lengths_sq_[index1] * lengths_sq_[index2]);
              }
              if (index2 == i){
                  grad_cov[i+ (m+1)*dim_ + (n+1)*dim_*(length_one+1)] += (point_two[index1] - point_one[index1])/
                                                                         (lengths_sq_[index1] * lengths_sq_[index2]);
              }
              grad_cov[i+ (m+1)*dim_ + (n+1)*dim_*(length_one+1)] * = cov;
          }
      }
  }
}
...
/*
  ...
  output:
  double: dim * (length_one+1) * (length_two+1)
*/
void SquareExponential::HyperparameterGradCovariance(double const * restrict point_one, int const * restrict derivatives_one, int length_one,
                                                     double const * restrict point_two, int const * restrict derivatives_two, int length_two,
                                                     double * restrict grad_hyperparameter_cov) const noexcept {
  double * cov_matrix = new double[(length_one+1)*(length_two+1)]();
  Covariance(point_one, derivatives_one, length_one, point_two, derivatives_two, length_two, cov_matrix);
  ...
  for (int n = 0; n < length_two; ++n){
      grad_hyperparameter_cov[(n+1)*(dim_+1)*(length_one+1)] = cov_matrix[(n+1)*(length_one+1)]/alpha_;
      index2 = derivatives_two[n];
      for (int i = 0; i < dim_; ++i){
          grad_hyperparameter_cov[i+1+(n+1)*(dim_+1)*(length_one+1)] = cov_matrix[(n+1)*(length_one+1)]*
                                                                       Square((point_one[i] - point_two[i])/lengths_[i])/lengths_[i];
          if (index2 == i){
              grad_hyperparameter_cov[i+1+(n+1)*(dim_+1)*(length_one+1)] -= (2*cov_matrix[0]*(point_one[i]-point_two[i]/length_sq_[i]))/length_[i];
          }
      }
  }
  ...
  for (int m = 0; m < length_one; ++m){
      ...
      for (int n = 0; n < length_two; ++n){
          index2 = derivatives_two[n];
          grad_hyperparameter_cov[(m+1)*(dim_+1)+(n+1)*(dim_+1)*(length_one+1)] = cov_matrix[m+1+(n+1)*(length_one+1)]/alpha_;
          for (int i = 0; i < dim_; ++i){
              grad_hyperparameter_cov[i+1+(m+1)*(dim_+1)+(n+1)*(dim_+1)*(length_one+1)] = cov_matrix[m+1+(n+1)*(length_one+1)]*
                                                                                          Square((point_one[i] - point_two[i])/lengths_[i])/lengths_[i];
              if (index1 == index2){
                  if (index1 == i){
                      grad_hyperparameter_cov[i+1+(m+1)*(dim_+1)+(n+1)*(dim_+1)*(length_one+1)] += ((4*cov_matrix[0]*Square(point_on[i]-point_two[i])/
                                                                                                   length_sq_[i])/lengths_sq_[i])/length_[i];
                      grad_hyperparameter_cov[i+1+(m+1)*(dim_+1)+(n+1)*(dim_+1)*(length_one+1)] -= (2*cov_matrix[0]/lengths_sq_[i])/length_[i];
                  }
              }
              else{
                  if (index1 == i){
                      grad_hyperparameter_cov[i+1+(m+1)*(dim_+1)+(n+1)*(dim_+1)*(length_one+1)] -= (2*cov_matrix[(n+1)*(length_one+1)]*
                                                                                                   (point_two[i]-point_one[i])/length_sq_[i])/length_[i];
                  }
                  if (index2 == i){
                      grad_hyperparameter_cov[i+1+(m+1)*(dim_+1)+(n+1)*(dim_+1)*(length_one+1)] -= (2*cov_matrix[m+1]*
                                                                                                   (point_one[i]-point_two[i])/length_sq_[i])/length_[i];
                  }
              }
          }
      }

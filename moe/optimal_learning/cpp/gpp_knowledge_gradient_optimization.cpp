...
//    std::copy(kg_state->normals.data() + i*num_union*(1+num_gradients_to_sample),
//              kg_state->normals.data() + (i+1)*num_union*(1+num_gradients_to_sample),
//              make_up_function_value.data());
//
//    std::vector<double> temp(num_union*(1+num_gradients_to_sample));
//    gaussian_process_->ComputeMeanOfAdditionalPoints(kg_state->union_of_points.data(), num_union, kg_state->gradients.data(),
//                                                     kg_state->num_gradients_to_sample, temp.data());
//    TriangularMatrixVectorMultiply(kg_state->cholesky_to_sample_var.data(), 'N',
//                                   num_union*(1+num_gradients_to_sample), make_up_function_value.data());
//    for (int val = 0; val < num_union*(1+num_gradients_to_sample); ++val){
//      make_up_function_value[val] += temp[val];
//    }

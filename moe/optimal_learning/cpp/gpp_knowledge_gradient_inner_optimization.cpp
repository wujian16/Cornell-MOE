...
void FuturePosteriorMeanEvaluator::ComputeGradPosteriorMean(
  ...
  MatrixTranspose(grad_cov, dim_, num_to_sample_*(1+num_derivatives_), grad_cov);
  TriangularMatrixMatrixSolve(chol_.data(), 'N', num_to_sample_*(1+num_derivatives_),
                              dim_, num_to_sample_*(1+num_derivatives_), grad_cov);
  GeneralMatrixVectorMultiply(grad_cov, 'T', coeff_.data(), 1.0, 1.0,
                              num_to_sample_*(1+num_derivatives_), dim_,
                              num_to_sample_*(1+num_derivatives_), grad_mu);

...
double ComputeLogLikelihoodWrapper(const boost::python::list& points_sampled,
  ...
  const int num_nn_hypers = 50*dim + 50 +
                            50*50 + 50 +
                            50*10 + 10;

  const int num_ak_hypers = 1+10;
  ...
  std::vector<double> hypers(num_nn_hypers + num_ak_hypers);
  std::copy(nn_hypers.data(), nn_hypers.data()+num_nn_hypers, hypers.data());
  std::copy(ak_hypers.data(), ak_hypers.data()+num_ak_hypers, hypers.data()+num_nn_hypers);

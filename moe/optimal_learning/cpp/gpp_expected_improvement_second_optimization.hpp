...
struct FutureValueFunctionState;
/*!\rst
...
\endrst*/
class FutureValueFunctionEvaluator final {
 public:
  ...
  \endrst*/
  void ComputeGradObjectiveFunction(StateType * vf_state, double * restrict grad_PS) const OL_NONNULL_POINTERS {
    ComputeGradValueFunction(vf_state, grad_PS);
  }
  ...
  \endrst*/
  double ComputeValueFunction(StateType * vf_state) const;
  ...
  \endrst*/
  void ComputeGradValueFunction(StateType * vf_state, double * restrict grad_PS) const;

  OL_DISALLOW_DEFAULT_AND_COPY_AND_ASSIGN(FutureValueFunctionEvaluator);
...
\endrst*/
struct FutureValueFunctionState final {
  using EvaluatorType = FutureValueFunctionEvaluator;
  ...
  void Initialize(const EvaluatorType& vf_evaluator);
  ...
  \endrst*/
  void SetCurrentPoint(const EvaluatorType& vf_evaluator,
  ...
  /*!\rst
    ...
    \param
      ...
      :point_to_sample[dim]: potential future sample whose EI (and/or gradients) is being evaluated
  \endrst*/
  void SetupState(const EvaluatorType& vf_evaluator,
                  double const * restrict point_to_sample_in) OL_NONNULL_POINTERS;
...
/*!\rst
  Set up vector of FutureValueFunctionState::StateType.
...
\endrst*/
inline OL_NONNULL_POINTERS void SetupFutureValueFunctionState(
    const FutureValueFunctionEvaluator& fpm_evaluator,
    double const * restrict points_to_sample,
    int max_num_threads,
    bool configure_for_gradients,
    const int num_fidelity,
    std::vector<typename FutureValueFunctionEvaluator::StateType> * state_vector) {
...
void RestartedGradientDescentFutureValueFunctionOptimization(const GaussianProcess& gaussian_process, const double best_so_far, const int num_fidelity,
  ...
  typename FutureValueFunctionEvaluator::StateType vf_state(vf_evaluator, num_fidelity, initial_guess, configure_for_gradients);

  GradientDescentOptimizer<FutureValueFunctionEvaluator, DomainType> gd_opt;
  gd_opt.Optimize(vf_evaluator, optimizer_parameters, domain, &vf_state);
  vf_state.GetCurrentPoint(best_next_point);
  best_objective_value = vf_evaluator.ComputeObjectiveFunction(&vf_state);
}

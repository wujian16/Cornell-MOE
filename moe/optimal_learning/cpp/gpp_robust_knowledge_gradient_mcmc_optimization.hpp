...
#ifndef MOE_OPTIMAL_LEARNING_CPP_GPP_ROBUST_KNOWLEDGE_GRADIENT_MCMC_OPTIMIZATION_HPP_
#define MOE_OPTIMAL_LEARNING_CPP_GPP_ROBUST_KNOWLEDGE_GRADIENT_MCMC_OPTIMIZATION_HPP_
...
struct RobustKnowledgeGradientMCMCState;
/*!\rst
...
\endrst*/
template <typename DomainType>
class RobustKnowledgeGradientMCMCEvaluator final {
 public:
  using StateType = RobustKnowledgeGradientMCMCState<DomainType>;
  /*!\rst
  ...
  \endrst*/
  explicit RobustKnowledgeGradientMCMCEvaluator(const GaussianProcessMCMC& gaussian_process_mcmc, const int num_fidelity,
  ...
  std::vector<RobustKnowledgeGradientEvaluator<DomainType>> * robust_knowledge_gradient_evaluator_list() const noexcept OL_WARN_UNUSED_RESULT {
    return robust_knowledge_gradient_evaluator_lst;
  }
  ...
  OL_DISALLOW_DEFAULT_AND_COPY_AND_ASSIGN(RobustKnowledgeGradientMCMCEvaluator);
 ...
 private:
  ...
  //! pointer to gaussian process used in KG computations
  std::vector<typename RobustKnowledgeGradientState<DomainType>::EvaluatorType> * robust_knowledge_gradient_evaluator_lst;
...
extern template class RobustKnowledgeGradientMCMCEvaluator<TensorProductDomain>;
extern template class RobustKnowledgeGradientMCMCEvaluator<SimplexIntersectTensorProductDomain>;

/*!\rst
  State object for RobustKnowledgeGradientEvaluator.  This tracks the points being sampled in concurrent experiments
...
\endrst*/
template <typename DomainType>
struct RobustKnowledgeGradientMCMCState final {
  using EvaluatorType = RobustKnowledgeGradientMCMCEvaluator<DomainType>;
  ...
  \endrst*/
  explicit RobustKnowledgeGradientMCMCState(const EvaluatorType& vf_evaluator, double const * restrict points_to_sample,
  ...
  RobustKnowledgeGradientMCMCState(RobustKnowledgeGradientMCMCState&& other);
  ...
  std::vector<typename RobustKnowledgeGradientEvaluator<DomainType>::StateType> * vf_state_list;

  OL_DISALLOW_DEFAULT_AND_COPY_AND_ASSIGN(RobustKnowledgeGradientMCMCState);
};
extern template struct RobustKnowledgeGradientMCMCState<TensorProductDomain>;
extern template struct RobustKnowledgeGradientMCMCState<SimplexIntersectTensorProductDomain>;
...
\endrst*/
template <typename DomainType>
inline OL_NONNULL_POINTERS void SetupRobustKnowledgeGradientMCMCState(
    const RobustKnowledgeGradientMCMCEvaluator<DomainType>& vf_evaluator,
    ...
    std::vector<typename RobustKnowledgeGradientEvaluator<DomainType>::StateType> * vf_state_vector,
    std::vector<typename RobustKnowledgeGradientMCMCEvaluator<DomainType>::StateType> * state_vector) {
...
void RestartedGradientDescentVFMCMCOptimization(const RobustKnowledgeGradientMCMCEvaluator& vf_evaluator,
  ...
  typename RobustKnowledgeGradientMCMCEvaluator::StateType vf_state(vf_evaluator, initial_guess,
  ...
  GradientDescentOptimizer<RobustKnowledgeGradientMCMCEvaluator, RepeatedDomain> gd_opt;
...
OL_NONNULL_POINTERS void ComputeVFMCMCOptimalPointsToSampleViaMultistartGradientDescent(
  ...
  std::vector<typename RobustKnowledgeGradientMCMCEvaluator<DomainType>::StateType> state_vector;
  std::vector<std::vector<typename RobustKnowledgeGradientEvaluator<DomainType>::StateType>> vf_state_vector(thread_schedule.max_num_threads);
  SetupRobustKnowledgeGradientMCMCState(vf_evaluator, start_point_set, points_being_sampled,
  ...
  using RepeatedDomain = RepeatedDomain<DomainType>;
  RepeatedDomain repeated_domain(domain, num_to_sample);
  GradientDescentOptimizer<RobustKnowledgeGradientMCMCEvaluator<DomainType>, RepeatedDomain> gd_opt;
  MultistartOptimizer<GradientDescentOptimizer<RobustKnowledgeGradientMCMCEvaluator<DomainType>, RepeatedDomain> > multistart_optimizer;
...
void EvaluateVFMCMCAtPointList(GaussianProcessMCMC& gaussian_process_mcmc, const int num_fidelity,
    ...
    std::vector<typename RobustKnowledgeGradientMCMCEvaluator<DomainType>::StateType> state_vector;
    std::vector<std::vector<typename RobustKnowledgeGradientEvaluator<DomainType>::StateType>> vf_state_vector(thread_schedule.max_num_threads);
    SetupRobustKnowledgeGradientMCMCState(vf_evaluator, initial_guesses, points_being_sampled,
                                    num_to_sample, num_being_sampled, num_pts, derivatives.data(), num_derivatives,
                                    thread_schedule.max_num_threads, configure_for_gradients,
                                    normal_rng, vf_state_vector.data(), &state_vector);
    ...
    NullOptimizer<RobustKnowledgeGradientMCMCEvaluator<DomainType>, DomainType_dummy> null_opt;
    typename NullOptimizer<RobustKnowledgeGradientMCMCEvaluator<DomainType>, DomainType_dummy>::ParameterStruct null_parameters;
    MultistartOptimizer<NullOptimizer<RobustKnowledgeGradientMCMCEvaluator<DomainType>, DomainType_dummy> > multistart_optimizer;
...
#endif  // MOE_OPTIMAL_LEARNING_CPP_GPP_ROBUST_KNOWLEDGE_GRADIENT_MCMC_OPTIMIZATION_HPP_
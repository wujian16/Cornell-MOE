...
#include "gpp_two_step_expected_improvement_mcmc_optimization.hpp"
...
TwoStepExpectedImprovementMCMCEvaluator<DomainType>::TwoStepExpectedImprovementMCMCEvaluator(const GaussianProcessMCMC& gaussian_process_mcmc,
...
: dim_(gaussian_process_mcmc.dim()),
  ...
  two_step_expected_improvement_evaluator_lst(evaluator_vector),
  discrete_pts_lst_(discrete_points_list(discrete_pts_lst, num_pts)),
  num_pts_(num_pts) {
    two_step_expected_improvement_evaluator_lst->reserve(num_mcmc_hypers_);
...
\endrst*/
template <typename DomainType>
double TwoStepExpectedImprovementMCMCEvaluator<DomainType>::ComputeCost(StateType * vf_state) const {
  if (num_fidelity_ == 0){
    return 1.0;
  }
  else{
...
\endrst*/
template <typename DomainType>
void TwoStepExpectedImprovementMCMCEvaluator<DomainType>::ComputeGradCost(StateType * vf_state, double * restrict grad_cost) const {
...
\endrst*/
template <typename DomainType>
double TwoStepExpectedImprovementMCMCEvaluator<DomainType>::ComputeValueFunction(StateType * vf_state) const {
  double vf_value = 0.0;
  for (int i=0; i<num_mcmc_hypers_; ++i){
    vf_value += (*two_step_expected_improvement_evaluator_lst)[i].ComputeObjectiveFunction((*(vf_state->vf_state_list)).data()+i);
  }
...
/*!\rst
  Computes gradient of KG (see TwoStepExpectedImprovementEvaluator::ComputeGradTwoStepExpectedImprovement) wrt points_to_sample (stored in
...
\endrst*/
template <typename DomainType>
void TwoStepExpectedImprovementMCMCEvaluator<DomainType>::ComputeGradValueFunction(StateType * vf_state, double * restrict grad_VF) const {
  double VF = 0.0;
  for (int i=0; i<num_mcmc_hypers_; ++i){
    std::vector<double> temp(vf_state->dim*vf_state->num_to_sample, 0.0);
    VF += (*two_step_expected_improvement_evaluator_lst)[i].ComputeGradValueFunction((*(vf_state->vf_state_list)).data()+i, temp.data());
...
template class TwoStepExpectedImprovementMCMCEvaluator<TensorProductDomain>;
template class TwoStepExpectedImprovementMCMCEvaluator<SimplexIntersectTensorProductDomain>;

template <typename DomainType>
void TwoStepExpectedImprovementMCMCState<DomainType>::SetCurrentPoint(const EvaluatorType& vf_evaluator,
  ...
  for (int i=0; i<vf_evaluator.num_mcmc();++i){
    (vf_state_list->at(i)).SetCurrentPoint(vf_evaluator.two_step_expected_improvement_evaluator_list()->at(i), points_to_sample_in);
  }
}

template <typename DomainType>
TwoStepExpectedImprovementMCMCState<DomainType>::TwoStepExpectedImprovementMCMCState(const EvaluatorType& vf_evaluator, double const * restrict points_to_sample,
  ...
  // evaluate derived quantities for the GP
  for (int i=0; i<vf_evaluator.num_mcmc();++i){
    vf_state_list->emplace_back(vf_evaluator.two_step_expected_improvement_evaluator_list()->at(i), points_to_sample, points_being_sampled,
...
template <typename DomainType>
TwoStepExpectedImprovementMCMCState<DomainType>::TwoStepExpectedImprovementMCMCState(TwoStepExpectedImprovementMCMCState&& OL_UNUSED(other)) = default;

template <typename DomainType>
void TwoStepExpectedImprovementMCMCState<DomainType>::SetupState(const EvaluatorType& vf_evaluator,
...
template struct TwoStepExpectedImprovementMCMCState<TensorProductDomain>;
template struct TwoStepExpectedImprovementMCMCState<SimplexIntersectTensorProductDomain>;

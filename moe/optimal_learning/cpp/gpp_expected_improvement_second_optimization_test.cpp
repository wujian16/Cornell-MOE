...
\endrst*/
class PingSecondValueFunction final : public PingableMatrixInputVectorOutputInterface {
 public:
  constexpr static char const * const kName = "the second value function in a Two-Step BO";

  PingSecondValueFunction(double const * restrict lengths, double const * restrict points_sampled, double const * restrict points_sampled_value,
                          int const * restrict gradients, double const * restrict points_to_sample, double best_so_far_,
      ...
      : dim_(dim),
        ...
        chol_(chol()),
        train_sample_(train()),
        vf_evaluator_(gaussian_process_, best_so_far_, normals_.data(), points_to_sample_.data(), num_to_sample_,
                      gradients_.data(), num_gradients_,
  ...
  virtual void EvaluateAndStoreAnalyticGradient(double const * restrict points_to_sample, double * restrict gradients) noexcept override OL_NONNULL_POINTERS_LIST(2) {
    ...
    FutureValueFunctionEvaluator::StateType vf_state(vf_evaluator_, 0, points_to_sample, configure_for_gradients);
    vf_evaluator_.ComputeGradValueFunction(&vf_state, grad_PS_.data());
  ...
  virtual void EvaluateFunction(double const * restrict points_to_sample, double * restrict function_values) const noexcept override OL_NONNULL_POINTERS {
    ...
    FutureValueFunctionEvaluator::StateType vf_state(vf_evaluator_, 0, points_to_sample, configure_for_gradients);
    *function_values = vf_evaluator_.ComputeValueFunction(&vf_state);
  }
 ...
 private:
  ...
  FutureValueFunctionEvaluator vf_evaluator_;

  OL_DISALLOW_DEFAULT_AND_COPY_AND_ASSIGN(PingSecondValueFunction);
};
...
\endrst*/
template <typename VFEvaluator>
OL_WARN_UNUSED_RESULT int PingVFTest(int num_to_sample, double epsilon[2], double tolerance_fine, double tolerance_coarse, double input_output_ratio) {
  int total_errors = 0;
  ...
  for (int i = 0; i < 50; ++i) {
    ...
    VFEvaluator vf_evaluator_(lengths.data(), KG_environment.points_sampled(), KG_environment.points_sampled_value(),
                              gradients, KG_environment.points_being_sampled(), best_so_far,
    ...
    vf_evaluator_.EvaluateAndStoreAnalyticGradient(KG_environment.points_to_sample(), nullptr);

    errors_this_iteration = PingDerivative(vf_evaluator_, KG_environment.points_to_sample(), epsilon, tolerance_fine, tolerance_coarse, input_output_ratio);
  ...
  if (total_errors != 0) {
    OL_PARTIAL_FAILURE_PRINTF("%s (%d,%d-VF) gradient pings failed with %d errors\n", VFEvaluator::kName, num_to_sample, 0, total_errors);
  } else {
    OL_PARTIAL_SUCCESS_PRINTF("%s (%d,%d-VF) gradient pings passed\n", VFEvaluator::kName, num_to_sample, 0);
  }
  delete [] gradients;
  return total_errors;
};
...
int Ping2EISecondGeneralTest() {
  double epsilon_KG[2] = {1.0e-3, 1.0e-4};

  int total_errors = PingVFTest<PingSecondValueFunction>(1, epsilon_KG, 9.0e-2, 3.0e-1, 1.0e-18);

  return total_errors;
}


int Run2EISecondTests() {
  int total_errors = 0;
  int current_errors = 0;

  {
    current_errors = Ping2EISecondGeneralTest();
    if (current_errors != 0) {
      OL_PARTIAL_FAILURE_PRINTF("pinging second EI failed with %d errors\n", current_errors);
    }
    total_errors += current_errors;
  }

  if (total_errors != 0) {
    OL_PARTIAL_FAILURE_PRINTF("Second EI functions failed with %d errors\n\n", total_errors);
  } else {
    OL_PARTIAL_SUCCESS_PRINTF("Second EI functions passed\n");
  }

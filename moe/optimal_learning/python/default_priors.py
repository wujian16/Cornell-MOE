...
class DefaultPrior(BasePrior):
    ...
    def __init__(self, n_dims, dim, num_noise):
        ...
        self.w = NormalPrior(mean=0.0, sigma=1.0)
    ...
    def lnprob(self, theta):
        lp = 0
        # weight
        for i in xrange(self.dim*self.dim+self.dim):
            lp += self.w.lnprob(theta[i])
        # Covariance amplitude
        for i in xrange(self.dim*self.dim+self.dim, self.dim*self.dim+2*self.dim):
            lp += self.ln_prior.lnprob(theta[i])
        # length scale
        for i in xrange(self.dim*self.dim+2*self.dim, self.dim*self.dim+3*self.dim):
            lp += self.tophat.lnprob(theta[i])
        # Noise
    ...
    def sample_from_prior(self, n_samples):
        ...
        ws_sample = np.array([self.w.sample_from_prior(n_samples)[:, 0]
                              for _ in xrange(self.dim*self.dim+self.dim)]).T
        p0[:, :self.dim*self.dim+self.dim] = ws_sample
        # Covariance amplitude
        cs_sample = np.array([self.ln_prior.sample_from_prior(n_samples)[:, 0]
                       for _ in xrange(self.dim)]).T
        p0[:, (self.dim*self.dim+self.dim):(self.dim*self.dim+2*self.dim)] = cs_sample
        # length scale
        ls_sample = np.array([self.tophat.sample_from_prior(n_samples)[:, 0]
                              for _ in xrange(self.dim)]).T
        p0[:, (self.dim*self.dim+2*self.dim):(self.dim*self.dim+3*self.dim)] = ls_sample
        # Noise

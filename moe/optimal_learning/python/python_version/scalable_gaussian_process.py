...
class ScalableDeepKernel(GaussianProcessInterface):

    r"""Implementation of a GaussianProcess with scalable deep kernel strictly in Python & Tensorflow.
    ...
    def __init__(self, deep_kernel, historical_data):
    ...
    def get_covariance_copy(self):
        ...
        """
        return DeepKernel(self._covariance.get_hyperparameters())
    ...
    def _build_precomputed_data(self):
        ...
        if self.num_sampled == 0:
            ...
            covariance_matrix = self._covariance.covariance(self._points_sampled)
            self._K_chol = tf.cholesky(covariance_matrix)
            self._K_inv_y = tf.cholesky_solve(self._K_chol, self._points_sampled_value)
    ...
    def compute_mean_of_points(self, points_to_sample):
        ...
        :type points_to_sample: tf.tensor of float64 with shape (num_to_sample, dim)
        :return: mean: where mean[i] is the mean at points_to_sample[i]
        :rtype: tensor of float64 with shape (num_to_sample, 1)

        """
        if self.num_sampled == 0:
            return numpy.zeros(points_to_sample.shape[0])

        K_star = self._covariance.covariance(points_to_sample, self._points_sampled)
        mu_star = tf.matmul(K_star, self._K_inv_y)
        return mu_star[:,0]
    ...
    def compute_grad_mean_of_points(self, points_to_sample, num_derivatives=-1):
        ...
        grad_K_star = []
        #tf.zeros((num_derivatives, self._points_sampled.shape[0], self.dim))
        for i, point_one in enumerate(points_to_sample[:num_derivatives, ...]):
            temp = []
            for j, point_two in enumerate(self._points_sampled):
                temp.append(self._covariance.grad_covariance(point_one, point_two))
            grad_K_star.append(tf.stack(temp))
        grad_K_star = tf.stack(grad_K_star)
        # y_{k,i} = A_{k,j,i} * x_j
        grad_mu_star = tf.einsum('ijk,j->ik', grad_K_star, self._K_inv_y[:,0])
        return grad_mu_star
    ...
    def compute_variance_of_points(self, points_to_sample):
        ...
        """
        var_star = self._covariance.covariance(points_to_sample) # this is K_star_star
        if self.num_sampled == 0:
            return tf.diag(tf.diag(var_star))

        K_star = self._covariance.covariance(self._points_sampled, points_to_sample)
        V = tf.cholesky_solve(
                self._K_chol,
                K_star,
        )
        ...
        var_star -= tf.matmul(tf.transpose(V), V)
        return var_star
    ...
    def _compute_grad_variance_of_points_per_point(self, points_to_sample, var_of_grad):
        ...
        #grad_var = tf.zeros((num_to_sample, num_to_sample, self.dim))

        K_star = self._covariance.covariance(self._points_sampled, points_to_sample)

        K_inv_times_K_star = tf.cholesky_solve(self._K_chol, K_star)

        grad_var = []
        for i, point_one in enumerate(points_to_sample):
            temp_i = []
            for j, point_two in enumerate(points_to_sample):
                temp = tf.zeros(self.dim)
                if var_of_grad == i and var_of_grad == j:
                    temp = self._covariance.grad_covariance(point_one, point_two)
                    for idx_two, sampled_two in enumerate(self._points_sampled):
                        temp -= 2.0 * K_inv_times_K_star[idx_two, i] * self._covariance.grad_covariance(point_one, sampled_two)
                elif var_of_grad == i:
                    temp = self._covariance.grad_covariance(point_one, point_two)
                    for idx_two, sampled_two in enumerate(self._points_sampled):
                        temp -= K_inv_times_K_star[idx_two, j] * self._covariance.grad_covariance(point_one, sampled_two)
                elif var_of_grad == j:
                    temp = self._covariance.grad_covariance(point_one, point_two)
                    for idx_one, sampled_one in enumerate(self._points_sampled):
                        temp -= K_inv_times_K_star[idx_one, i] * self._covariance.grad_covariance(point_two, sampled_one)
                temp_i.append(temp)
            grad_var.append(tf.stack(temp_i))
        return tf.stack(grad_var)
    ...
    def compute_grad_variance_of_points(self, points_to_sample, num_derivatives=-1):
        ...
        """
        num_derivatives = self._clamp_num_derivatives(points_to_sample.shape[0], num_derivatives)
        grad_var = [] #tf.zeros((num_derivatives, points_to_sample.shape[0], points_to_sample.shape[0], self.dim))
        for i in xrange(num_derivatives):
            grad_var.append(self._compute_grad_variance_of_points_per_point(points_to_sample, i))
        return tf.stack(grad_var)
    ...
    def _compute_grad_cholesky_variance_of_points_per_point(self, points_to_sample, chol_var, var_of_grad):
        ...
        grad_chol = self._compute_grad_variance_of_points_per_point(points_to_sample, var_of_grad).eval()
    ...
    def compute_grad_cholesky_variance_of_points(self, points_to_sample, chol_var=None, num_derivatives=-1):
        ...
        if chol_var is None:
            ...
            chol_var = self.compute_cholesky_variance_of_points(points_to_sample).eval()

        grad_chol_decomp = numpy.zeros((num_derivatives, points_to_sample.shape[0], points_to_sample.shape[0], self.dim))
        for i in xrange(num_derivatives):

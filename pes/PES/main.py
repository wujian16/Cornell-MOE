...
def run_PES(target_function, x_minimum, x_maximum, dimension, number_of_hyperparameter_sets = 100, number_of_burnin = 50, \
    ...
    for pp in range(number_of_experiments):
        ...
        with hide_prints():
            noise, l, sigma = sample_hypers(Xsamples, Ysamples, d, 0.3, num_of_hyperSets_initial, number_burn, sample_method)
        ...
        for g in range(number_of_iterations):
            ...
            print(pos_optimum)
            #guessed_value = target(pos_optimum)
            #print(guessed_value)
            print('guessed point above')
            

            guesses = np.vstack((guesses, pos_optimum))
            print('sucess',valid_evaluation)
            valid_evaluation = valid_evaluation + 1
